...
```{definition}
O *Efeito do Plano Amostral - $EPA$ de Kish* para um estimador $\widehat{\theta}$ é
```
...
Efeitos do plano amostral de Kish para estimadores de totais com amostragem conglomerada em dois estágios.
```
...
names(tabela) <- c('Variável', 'AC2S + Estimador HT', 'AC2S + Estimador de Razão', 'AC2PC + Estimador HT')

knitr::kable(tabela,align= "lrrr",format.args= list(big.mark = '.',dec=','),escape = F, booktabs=T,
caption= "\\centering $\\text{Efeitos do plano amostral de Kish para variáveis selecionadas}$ \\newline $\\text{Região Metropolitana do Rio de Janeiro}$", row.names=FALSE)%>%
...
Podemos desta forma introduzir uma medida de efeito da especificação incorreta do plano amostral (ou do modelo) sobre a estimativa $v_{0}$ da variância do estimador $\widehat{\theta}$.


```{definition}
O efeito da especificação incorreta do plano amostral (ou do modelo) sobre a estimativa $v_{0}$ da variância do estimador $\widehat{\theta}$ é
```
...
Efeitos do plano amostral para estimação de médias na amostragem estratificada simples com alocação desproporcional
```
...
c(tab_44[1,2], tab_44[1,2]/tab_44[3,2] ,tab_44[1,3], tab_44[1,3]/tab_44[3,3])
)
...
caption= "$\\text{Estimativas da variância e do efeito do plano amostral para as médias dos salários e receitas}$", row.names=FALSE)%>%
kable_styling(full_width = F,latex_options = 'HOLD_position')
```
...
names(tabela) <- c('$EPA \\left( \\widehat{\\theta}, v_0\\right)$', '$1-\\alpha=0,95$', '$1-\\alpha=0,99$')
...
Os autovalores $\delta_1 \geq \ldots \geq \delta_K$ da matriz $\boldsymbol{\Delta}$ são denominados *efeitos generalizados do plano amostral*. A partir deles, e utilizando resultados padrões de teoria das matrizes (@Johnson, página 64) é possível definir limites para os efeitos (univariados) do plano amostral para qualquer combinação linear $\mathbf{c}^{\prime} \widehat{\boldsymbol{\theta}}$ das componentes de $\widehat{\boldsymbol{\theta}}$. Segundo @Sk89a, página 43, temos os seguintes resultados:
...
Neste exemplo, a matriz de efeito do plano amostral $\mathbf{EMPA} \left(\boldsymbol{\widehat{\theta} , V}_0 \right) = \boldsymbol{\Delta}$ pode também ser calculada através de simulação, de modo análogo ao que foi feito no Exemplo \@ref(exm:exC4N2). Para isto, foram utilizadas as $500$ amostras de tamanho $60$ segundo o plano amostral descrito no Exemplo \@ref(exm:exC4N2). Para cada uma das $500$ amostras foram calculadas estimativas:
