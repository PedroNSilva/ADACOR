...
<span class="example" id="exm:pnad6"><strong>(\#exm:pnad6) </strong></span>Análise do perfil sócio-econômico das pessoas
...
\caption{(\#tab:varexp)$\text{Descrição das variáveis explicativas}$}
\centering
\begin{tabular}[t]{lcl}
\toprule
...
\caption{(\#tab:logit)$\text{Estmativas dos efeitos e respectivos erros padrões obtidos pela library survey do R}$}
\centering
