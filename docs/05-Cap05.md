...
detalhes pode consultar livros clássicos como @Cochran1977 ou mais
recentes como @SSW92. Já os métodos
requeridos para inferências analíticas só recentemente foram
consolidados em livro ver @SHS89. Este capítulo
...
paramétricos regulares ver, por exemplo, @garthwaite.
...
<span class="example" id="exm:pnad"><strong>(\#exm:pnad) </strong></span>Uso analítico dos dados da Pesquisa Nacional por Amostra de
Domicílios (PNAD)
...
menores dentro das UFs, ver @IBGE81, p. 67.
...
\caption{(\#tab:proselpnad)$\text{Probabilidades de seleção da amostra da PNAD de 1993 segundo regiões}$}
\centering
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{14cm}>{\centering\arraybackslash}p{2.5cm}}
\toprule
Região da pesquisa & Probabilidade de seleção\\
\midrule
RM de Belém & 1/150\\
RMs de Fortaleza, Recife, Salvador, Porto Alegre e Distrito Federal & 1/200\\
...
$w_{i}^{R}=\pi_{i}^{-1}X\,/\,\widehat{X}_{\pi }$,ver \@ref(eq:estpa17), onde $X$ é o total
...
<span class="definition" id="def:unnamed-chunk-1"><strong>(\#def:unnamed-chunk-1) </strong></span>O estimador de Máxima Pseudo-Verossimilhança (MPV) $\mathbf{\hat{\theta}}_{MPV}$ de $\mathbf{\theta }_{U}$ (e consequentemente de $\mathbf{\theta}$) será a solução das equações dePseudo-Verossimilhança dadas por 
\begin{equation}
...
\iffalse{} <span class="remark"><em>Observação </em></span>  \fi{}No Método de Máxima Pseudo-Verossimilhança, os pesos amostrais
...
\BeginKnitrBlock{remark}
\iffalse{} <span class="remark"><em>Observação </em></span>  \fi{}O plano amostral é também incorporado no método de estimação MPV através da expressão para a variância do total dos escores sob o plano amostral \@ref(eq:modpar14), onde as propriedades do plano amostral estão resumidas nas probabilidades de inclusão de primeira
e segunda ordem, isto é, os $\pi _{i}$ e os $\pi _{ij}$ respectivamente.
\EndKnitrBlock{remark}

\BeginKnitrBlock{remark}
\iffalse{} <span class="remark"><em>Observação </em></span>  \fi{}Sob probabilidades de seleção iguais, os pesos $\pi _{i}^{-1}$
...
\BeginKnitrBlock{remark}
\iffalse{} <span class="remark"><em>Observação </em></span>  \fi{}Consistência referente à distribuição de aleatorização.
\EndKnitrBlock{remark}
...
\BeginKnitrBlock{example}
<span class="example" id="exm:Analisec"><strong>(\#exm:Analisec) </strong></span>Análise secundária de tabelas de contingência.
\EndKnitrBlock{example}
