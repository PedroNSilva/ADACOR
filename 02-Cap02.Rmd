...
```{r,modclas, echo=FALSE, fig.cap="Representação esquemática da *Modelagem Clássica*",fig.pos="!H"}
...
l2=c('Modelo / Hipóteses','$\\text{Dados extraídos de }y_1,\\ldots, y_N \\text{ segundo }p(a)$')
...
```{r,modamo, echo=FALSE, fig.cap="Representação esquemática da *Amostragem Probabilística*"}
...
```{r, modsup, echo=FALSE, fig.cap="Representação esquemática da *Modelagem de Superpopulação*"}
...
l1=c('Dados Amostrais','$y_{i_1}, \\ldots, y_{i_n}$')
l2=c('População e esquema de seleção','$\\text{Selecionados de } y_1, \\dots, y_N \\text{ segundo } p(a)$')
...
l5=c('Objetivo','$\\text{Inferir sobre }g\\left( y_{1}, \\ldots , y_{N} \\right) \\text{ partir de } y_{i_1}, \\ldots, y_{i_n} \\text{ usando } p(a)$ ')
