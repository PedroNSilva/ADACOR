...
abordagem que aqui chamamos de *Modelagem clássica* considera $y_1, \ldots ,y_n$
...
+------------------------+-----------------------------------------------------+
| Modelo Paramétrico/    | $Y_1, \ldots ,Y_n$ variáveis aleatórias IID com     |
| Hipóteses              | distribuição $f(y,\theta)$, onde                    |
|                        | $\theta \in \Theta$                                 |
+------------------------+-----------------------------------------------------+
| Objetivo               | Inferir sobre $\theta$ usando as observações        |
|                        | $y_1, \ldots ,y_n$                                  |
+------------------------+-----------------------------------------------------+

: (\#tab:modelclass) Representação esquemática da abordagem *Modelagem
Clássica*.
...
Os valores $y_{1}, \ldots ,y_{N}$ das variáveis de interesse $Y$ na *população
...
denominadas *parâmetros da população finita* pelos amostristas. Em geral, o
...
| Hipóteses              |                                                     |
+------------------------+-----------------------------------------------------+
| Objetivo               | Inferir sobre funções $g(y_1, \ldots , y_N)$ usando |
|                        | $y_{i_1}, \ldots, y_{i_n}$                          |
+------------------------+-----------------------------------------------------+

: (\#tab:modelamo) Representação esquemática da abordagem *Amostragem
Probabilística*.
...
obtenção dos dados, através do *planejamento amostral* $p(a)$ utilizado
[@Neyman] e as distribuições das estatísticas de interesse são derivadas a
...
totais, médias, proporções, razões, etc. Sob a abordagem de *Amostragem
...
-   É adequado aplicar métodos de análise da *Modelagem Clássica*, concebidos
...
-   Ou seria mais adequado fazer uso analítico dos dados dentro da abordagem de
    *Amostragem Probabilística*? E neste caso, como fazer isto, visto que nesta
...
-   O que acontece quando o modelo adotado na *Modelagem Clássica* não é
    verdadeiro?

-   Neste caso, qual a interpretação dos parâmetros na *Modelagem Clássica*?

-   Ainda neste caso, as quantidades descritivas populacionais da *Amostragem
...
da estrutura do planejamento amostral, como feito habitualmente na *Amostragem
Probabilística*. Essa abordagem, denominada de *Modelagem de Superpopulação*,
foi primeiro proposta em [@Brewer1963] e [@royall], e é bem descrita, por
exemplo, em [@binder83] e [@valliant].
...
onde $\theta \in \Theta$. Este modelo é denominado `Modelo de Superpopulação`.
Note que, em contraste com o que se faz na *Modelagem Clássica*, o modelo
probabilístico é aqui especificado para descrever o mecanismo aleatório que gera
a *população*, não a amostra. Na maioria das aplicações práticas, a população de
...
estimar alguma coisa sobre toda a população", conforme [@thompson].
...
*Modelagem Clássica*) que são quantidades descritivas populacionais (parâmetros
...
*Amostragem Probabilística* para fazer inferência sobre
...
| Dados Amostrais        | $Y_1=y_{i_1},\ldots,Y_n=y_{i_n}$                    |
+------------------------+-----------------------------------------------------+
| População e esquema de | Extraídos de $y_1,\dots,y_N$ segundo $p(a)$         |
| seleção                |                                                     |
+------------------------+-----------------------------------------------------+
| Modelo para população  | $Y_1, \dots, Y_N$ variáveis aleatórias IID com      |
|                        | distribuição $f(y,\theta)$, onde                    |
|                        | $\theta \in \Theta$                                 |
+------------------------+-----------------------------------------------------+
| Parâmetro-alvo         | associar                                            |
|                        | $\theta \longl                                      |
|                        | e ftrightarrow g\left(Y_{1}, \ldots , Y_{N}\right)$ |
+------------------------+-----------------------------------------------------+
| Objetivo               | Inferir sobre                                       |
|                        | $g\left( Y_{1}, \ldots , Y_{N}\right)$ partir de    |
|                        | $y_{i_1},\ldots,y_{i_n}$ usando $p\left( a\right)$  |
+------------------------+-----------------------------------------------------+

: (\#tab:modelsuperpop) Representação esquemática da *Modelagem de
Superpopulação*.
...
do texto. Admitiremos que o leitor esteja familiarizado com a *Modelagem
Clássica* e com as noções básicas da *Amostragem Probabilística*. A título de
recordação, serão apresentados no Capítulo \@ref(planamo) alguns resultados
básicos da *Amostragem Probabilística*. A ênfase do texto, porém, será na
apresentação da *Modelagem de Superpopulação*, sendo para isto apresentados os
elementos indispensáveis das abordagens de *Modelagem Clássica* e da *Amostragem
Probabilística*.
...
*corretas* utilizando os pacotes tradicionais. Por outro lado, para construir
...
Ao longo deste livro faremos uso intensivo do pacote `survey` disponível no R,
...
tipo *aponte e clique* terão dificuldade adicional de adaptar-se à linguagem de
...
será usado no restante deste texto. [@cassel] sugerem que um referencial para
...
indicado, de agora em diante admitiremos que não há *erros de medição*,
implicando que os valores observados de quaisquer variáveis de interesse serão
considerados valores corretos ou verdadeiros. Admitiremos ainda que há *resposta
...
foi realizada. Hipóteses semelhantes são adotadas, por exemplo, em [@binder83] e
[@Mont87].

Portanto, o referencial aqui adotado considera apenas duas fontes alternativas
de variação: o *Modelo de Superpopulação* (1) e o *Plano Amostral* (3). Estas
...
Este modelo foi considerado em [@cassel], que o chamaram de *abordagem de
...
como [@cochran] e outros. Aqui esta abordagem é chamada de *abordagem baseada no
planejamento amostral* ou *abordagem de aleatorização*, pois neste caso a única
...
não precisa ser degenerada como em \@ref(eq:ref4), embora o referencial aqui
...
superpopulação é chamada *inferência analítica*. Este tipo de inferência só faz
sentido quando o modelo de superpopulação não é degenerado como em
\@ref(eq:ref4). Usualmente seu objetivo é explicar a relação entre variáveis não
...
*informativa*, veja Seção \@ref(inform). Uma notação mais explícita para indicar
...
das variáveis de pesquisa. Um exemplo típico desta situação é o dos *estudos de
caso-controle*, em que a amostra é selecionada de tal forma que há *casos*
(unidades com determinada condição) e *controles* (unidades sem essa condição),
...
os planos amostrais considerados são *não-informativos*, isto é, não podem
...
*distribuição de aleatorização*) para quantidades descritivas da população
...
então feitas com respeito à *distribuição de aleatorização* das estatísticas
...
: (\#tab:Tab24) Distribuição de probabilidades conjunta na população
$Pr( Y_i = y ; X_i = x )$.
...
correspondente ao valor $x$ usado como índice. Esta alocação é dita *alocação
...
: (\#tab:Tab25) Distribuição de probabilidades condicional de $y$ dado $x$ na
...
\end{eqnarray}
```
Dado o plano amostral acima descrito, a distribuição marginal de $X$ *na
...
Podemos usar a informação sobre a distribuição condicional de $Y$ dado $X$ *na
população* e a informação sobre a distribuição marginal de $X$ *na amostra* para
...
na população ser a mesma e que a distribuição condicional de $Y$ dado $X$ *na
amostra*.
...
: (\#tab:Tab26) Distribuição de probabilidades conjunta na população
$f_U( x ; y )$.
...
: (\#tab:Tab27) Distribuição de probabilidades condicional de $Y$ dado $X$ *na
população* - $f_U( y | x )$.
...
: (\#tab:Tab28) Distribuição de probabilidades marginal de $Y$ *na população* -
$f_U( y )$.
...
de $Y$ dado $X$, a amostragem seria *ignorável*, isto é,
